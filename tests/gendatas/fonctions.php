...
{
    $req = 'select * from utilisateur WHERE idprofil = 1';
    $res = $pdo->query($req);
...
/**
 * Fonction qui met à jour les mots de passe des utilisateurs avec des mots de
 * pass aléatoire
 *
 * @param PDO $pdo instance de la classe PDO utilisée pour se connecter
 *
...
function updateMdpVisiteur($pdo)
{
    ...
    foreach ($lesLignes as $unVisiteur) {
        ...
        $req = "update utilistaeur set mdp ='$mdp' where utilisateur.id ='$id' ";
        $pdo->exec($req);
    }
}
